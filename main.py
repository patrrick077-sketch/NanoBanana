...
def zcheck(zuid, zcid, zmid=None):
    ...
    try:
        ...
        # If the bot is not an admin in the channel, get_chat_member will fail.
        print(f"Membership check failed (Ensure bot is admin in both channels): {e}")
        
    # If the user is NOT in the channels, send the warning
    try:
        if zmid:
            zeco.delete_message(zcid, zmid) # Delete their triggered message/button
    except:
        pass
...
def zcb(zc):
    ...
    if zdata == 'zcheck':
        if zcheck(zuid, zcid, zmid):
            # If they actually joined, delete auth message and show menu
            try:
    ...
    # Intercept ALL other buttons and verify membership first
...
def zezeze3(zmsg, zuid):
    ...
    # Check membership right before processing generation
    if not zcheck(zuid, zcid, zmsg.message_id):
        return
    ...
    zresp = zapi({'text': zmsg.text, 'model': zapim, 'ratio': zrat, 'res': zres})
    try:
...
def zezeze2(zmsg, zuid):
    # Check membership before accepting the image
...
def zezeze1(zmsg, zuid):
    ...
    # Check membership right before processing edit generation
    if not zcheck(zuid, zcid, zmsg.message_id):
        return
    ...
    zresp = zapi({'text': zmsg.text, 'model': zapim, 'links': zimg, 'ratio': zrat, 'res': zres})
    try:
        zeco.delete_message(zcid, zsid)
    except:
        pass
